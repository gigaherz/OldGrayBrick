...
#define SET_FLAG_B(v)   SET_FLAG(4,v)
...
int Cpu::Step()
{
	...
	case 0x85: // STA nn      Store A in Zero Page     [nn]=A
		memory->Write(Fetch(),A); // nn
		cycles = 3; //  
		break;
	case 0x95: // STA nn,X    Store A in Zero Page,X   [nn+X]=A
		memory->Write((Fetch()+X)&0xFF,A); // nn,X
		cycles = 4; //  
		break;
	...
	case 0x81: // STA (nn,X)  Store A in (Indirect,X)  [[nn+x]]=A
		a1 = Fetch()+X; // (nn,X)
		a2 = memory->Read(a1&0xFF)|(((u16)memory->Read((a1+1)&0xFF))<<8);
		memory->Write(a2,A);
		cycles = 6; //  
		break;
	...
	case 0x86: // STX nn      Store X in Zero Page     [nn]=X
		memory->Write(Fetch(),X); // nn
		cycles = 3; //  
		break;
	case 0x96: // STX nn,Y    Store X in Zero Page,Y   [nn+Y]=X
		memory->Write((Fetch()+Y)&0xFF,X); // nn,Y
		cycles = 4; //  
		break;
	...
	case 0x84: // STY nn      Store Y in Zero Page     [nn]=Y
		memory->Write(Fetch(),Y); // nn
		cycles = 3; //  
		break;
	case 0x94: // STY nn,X    Store Y in Zero Page,X   [nn+X]=Y
		memory->Write((Fetch()+X)&0xFF,Y); // nn,X
		cycles = 4; //  
		break;
	...
	case 0x69: // ADC #nn     Add Immediate           A=A+C+nn
		p1 = Fetch(); // #nn
		new_A=(s32)(s8)A+(s32)(s8)p1+FLAG_C;
		cycles = 2; //  
	...
	case 0x24: // BIT nn      Bit Test   A AND [nn], N=[nn].7, V=[nn].6
		...
		new_T = p1&A;
		cycles = 3; //  
		CALC_FLAG_N(new_T);CALC_FLAG_Z(new_T);SET_FLAG_V(new_T>>6);
		break;
